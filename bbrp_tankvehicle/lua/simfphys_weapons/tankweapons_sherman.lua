...
local function hmg_fire(ply,vehicle,shootOrigin,shootDirection)
	...
	simfphys.FireBullet( projectile )
end
...
local function cannon_fire(ply,vehicle,shootOrigin,shootDirection)
	...
	simfphys.FirePhysBullet( projectile )
end
...
function simfphys.weapon:Initialize( vehicle )
	...
	simfphys.xhairRegister( vehicle:GetDriverSeat(), { Direction = Vector(0,0,1),Attachment = "turret_cannon", Type = 2 } )
	simfphys.CameraRegister( vehicle:GetDriverSeat(), Vector(-30,0,0), Vector(20,60,65), true,"turret_machinegun" )
	
	if not istable( vehicle.PassengerSeats ) or not istable( vehicle.pSeat ) then return end

	simfphys.xhairRegister( vehicle.pSeat[1] , { Attachment = "machinegun", Type = 1 } )
	simfphys.CameraRegister( vehicle.pSeat[1], Vector(0,-40,5), Vector(0,-40,50), true )
	
	simfphys.CameraRegister( vehicle.pSeat[2], Vector(0,0,25), Vector(0,0,25), false )
	
	simfphys.CameraRegister( vehicle.pSeat[3], Vector(0,0,55), Vector(0,0,55), true )
		...
		if Toggle ~= vehicle.OldToggleC then
			...
			if Toggle then
				...
				if vehicle.tg_c_z then
					...
					simfphys.CameraRegister( Commander, Vector(0,0,0), Vector(0,0,0), false )
				else
					vehicle:EmitSound( "vehicles/atv_ammo_close.wav" )
					simfphys.CameraRegister( Commander, Vector(0,0,25), Vector(0,0,25), false )
				end
			end
		end
