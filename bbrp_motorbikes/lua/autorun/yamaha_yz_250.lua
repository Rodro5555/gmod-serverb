...
cheapbonemanips["NoRealistBikeYAMAHAYZ250"] = {
...
cheapbonemanips["InRealistBikeYAMAHAYZ250"] = {
...
hook.Add("PlayerEnteredVehicle","RealisticFixBoneYAMAHAYZ250",function( ply, veh, role )
	if veh:GetVehicleClass()=="realistic_bike_yamaha_yz_250" then
		for k,v in pairs(cheapbonemanips["InRealistBikeYAMAHAYZ250"]) do
...
hook.Add("PlayerLeaveVehicle","RealisticFixBoneYAMAHAYZ250",function( ply, veh )
	if veh:GetVehicleClass()=="realistic_bike_yamaha_yz_250" then
		for k,v in pairs(cheapbonemanips["NoRealistBikeYAMAHAYZ250"]) do
			local boneid = ply:LookupBone(k)
			if boneid then
				ply:ManipulateBonePosition(boneid,v[1])
				ply:ManipulateBoneAngles(boneid,v[2])
		...
		Helmet:Remove()
		ply:SetHaveHelmet_Yz_250(false)
	end
end)
...
function MOTARD:SetHaveHelmet_Yz_250(bool)
	self:SetNW2Bool("RealisticBike_HaveHelmet_Yz_250", bool)
end

function MOTARD:GetHaveHelmet_Yz_250()
...
function MOTARD:SetHelmetGlass_Yz_250(bool)
	self:SetNW2Bool("RealisticBike_HelmetGlass_Yz_250", bool)
end

function MOTARD:GetHelmetGlass_Yz_250()
	return self:GetNW2Bool("RealisticBike_HelmetGlass_Yz_250")
end



hook.Add("Think", "RealisticFixBoneYAMAHAYZ250", function()
		...
		if veh:GetVehicleClass()=="realistic_bike_yamaha_yz_250" then
			...
			if CLIENT then --CLIENT
				...
				if IsValid(veh:GetDriver()) then
					...
					if ply:GetHelmetGlass_Yz_250() then
					...
					if ply:GetHaveHelmet_Yz_250() then
...
hook.Add("VehicleMove", "RealisticFixBoneYAMAHAYZ250", function(ply, veh, mv)
	if not veh:GetVehicleClass()=="realistic_bike_yamaha_yz_250" then return end
	...
	if SERVER then
		...
		if Reload then
			if ply:GetHelmetGlass_Yz_250() then
				ply:SetHelmetGlass_Yz_250(false)
			elseif not ply:GetHelmetGlass_Yz_250() then
				ply:SetHelmetGlass_Yz_250(true)
			end
		end
		...
		if ShAR then
			if ply:GetHaveHelmet_Yz_250() then
				ply:SetHaveHelmet_Yz_250(false)
			elseif not ply:GetHaveHelmet_Yz_250() then
				ply:SetHaveHelmet_Yz_250(true)
			end			
		end
	end
end)
...
local V = { 	
				...
				Model = "models/realistic_bike/yamaha_yz_250.mdl",

				KeyValues = {
								vehiclescript	=	"scripts/vehicles/ceil-code/yamaha_yz_250.txt"
							},
				Members = {
								HandleAnimation = HandleBoatVehicleAnimation,
				}
			}
list.Set( "Vehicles", "realistic_bike_yamaha_yz_250", V )