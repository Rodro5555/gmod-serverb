...
cheapbonemanips["NoRealistBikeKAWANinjaH2"] = {
...
cheapbonemanips["InRealistBikeKAWANinjaH2"] = {
	[pelvis]             	= {Vector(-6,0,0),Angle(0,0,35)},
	[r_calf]             	= {Vector(0,0,0),Angle(20,50,0)},
	[l_calf]             	= {Vector(0,0,0),Angle(-20,50,0)},
	[r_thigh]            	= {Vector(0,0,0),Angle(20,-10,0)},
	[l_thigh]            	= {Vector(0,0,0),Angle(-20,-10,0)},
	[r_foot]             	= {Vector(0,0,0),Angle(0,-45,0)},
	[l_foot]             	= {Vector(0,0,0),Angle(0,-45,0)},
	[r_upperarm]         	= {Vector(0,0,0),Angle(13,-5,0)},
	[r_forearm]          	= {Vector(0,0,0),Angle(0,20,0)},
	[r_hand]             	= {Vector(0,0,0),Angle(-30,30,40)},
	[l_upperarm]         	= {Vector(0,0,0),Angle(-13,-5,0)},
	[l_forearm]          	= {Vector(0,0,0),Angle(0,20,0)},
	[l_hand]             	= {Vector(0,0,0),Angle(0,30,-40)},
...
hook.Add("PlayerEnteredVehicle","RealisticFixBoneKAWANinjaH2",function( ply, veh, role )
	if veh:GetVehicleClass()=="realistic_bike_kawasaki_ninja_h2" then
		for k,v in pairs(cheapbonemanips["InRealistBikeKAWANinjaH2"]) do
...
hook.Add("PlayerLeaveVehicle","RealisticFixBoneKAWANinjaH2",function( ply, veh )
	if veh:GetVehicleClass()=="realistic_bike_kawasaki_ninja_h2" then
		for k,v in pairs(cheapbonemanips["NoRealistBikeKAWANinjaH2"]) do
			local boneid = ply:LookupBone(k)
			if boneid then
				ply:ManipulateBonePosition(boneid,v[1])
				ply:ManipulateBoneAngles(boneid,v[2])
		...
		Helmet:Remove()
		ply:SetHaveHelmet_KAWA_NINJA_H2(false)
	end
end)
...
function MOTARD:SetHaveHelmet_KAWA_NINJA_H2(bool)
	self:SetNW2Bool("RealisticBike_HaveHelmet_KAWA_NINJA_H2", bool)
end

function MOTARD:GetHaveHelmet_KAWA_NINJA_H2()
...
function MOTARD:SetHelmetGlass_KAWA_NINJA_H2(bool)
	self:SetNW2Bool("RealisticBike_HelmetGlass_KAWA_NINJA_H2", bool)
end

function MOTARD:GetHelmetGlass_KAWA_NINJA_H2()
	return self:GetNW2Bool("RealisticBike_HelmetGlass_KAWA_NINJA_H2")
end


hook.Add("Think", "RealisticFixBoneKAWANinjaH2", function()
	...
	for k,veh in pairs(ents.FindByClass("prop_vehicle_jeep")) do
		if veh:GetVehicleClass()=="realistic_bike_kawasaki_ninja_h2" then
			...
			if CLIENT then --CLIENT
				...
				---HIDE Helmet
				if IsValid(Entity(LocalPlayer():GetHelmet())) then
					if LocalPlayer():GetHaveHelmet_KAWA_NINJA_H2() then
						if not veh:GetThirdPersonMode() then
							if veh == LocalPlayer():GetVehicle() then
								Entity(LocalPlayer():GetHelmet()):SetColor(Color(255,255,255,0))
							end
						else
							Entity(LocalPlayer():GetHelmet()):SetColor(Color(255,255,255,255))
						end
					end
				end
				...
				if IsValid(veh:GetDriver()) then
					...
					if ply:GetHelmetGlass_KAWA_NINJA_H2() then
					...
					if ply:GetHaveHelmet_KAWA_NINJA_H2() then
...
hook.Add("VehicleMove", "RealisticFixBoneKAWANinjaH2", function(ply, veh, mv)
	if not veh:GetVehicleClass()=="realistic_bike_kawasaki_ninja_h2" then return end
	...
	if SERVER then
		...
		if Reload then
			if ply:GetHelmetGlass_KAWA_NINJA_H2() then
				ply:SetHelmetGlass_KAWA_NINJA_H2(false)
			elseif not ply:GetHelmetGlass_KAWA_NINJA_H2() then
				ply:SetHelmetGlass_KAWA_NINJA_H2(true)
			end
		end
		...
		if ShAR then
			if ply:GetHaveHelmet_KAWA_NINJA_H2() then
				ply:SetHaveHelmet_KAWA_NINJA_H2(false)
			elseif not ply:GetHaveHelmet_KAWA_NINJA_H2() then
				ply:SetHaveHelmet_KAWA_NINJA_H2(true)
			end			
		end
	end
end)
...
local V = { 	
				...
				Model = "models/realistic_bike/kawasaki_ninja_h2r.mdl",

				KeyValues = {
								vehiclescript	=	"scripts/vehicles/ceil-code/kawasaki_ninja_h2.txt"
							},
				Members = {
								HandleAnimation = HandleBoatVehicleAnimation,
				}
			}
list.Set( "Vehicles", "realistic_bike_kawasaki_ninja_h2", V )