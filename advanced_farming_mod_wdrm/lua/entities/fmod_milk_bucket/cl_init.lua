...
function ENT:Draw()
	...
	local encsize = 1
	local perc = math.Clamp( self:GetMilk()/20 * 100, 0,100 )
	local perca = self:GetMilk() 
	...
	cam.Start3D2D(position, angle, 0.08)
		...
		draw.SimpleTextOutlined(FarmingMod.Config.Lang["Milk"][FarmingMod.Config.CurrentLang].." : "..perca.." L", "Trebuchet24" ,0,(1*20-20), Color(255,255,255,255), 1, 1, 0.5, Color(0,0,0,255))
	...
	cam.Start3D2D(position, angle, 0.1)
		...
		draw.RoundedBox( 130,-sizex/2,-sizey/2,sizex,sizey, Color(255,255,255,255) )
	cam.End3D2D()
end
