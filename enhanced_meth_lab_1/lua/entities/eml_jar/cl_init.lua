...
function ENT:Draw()
	...
	local potTime = "Progreso: "..self:GetNWInt("progress").."% (Shake it!)";
	
	if (self:GetNWInt("status") == 0) then
		potTime = "Progreso: "..self:GetNWInt("progress").."% (Shake it!)";
	elseif (self:GetNWInt("status") == 1) then	
		potTime = "¡Listo!¡Use para extraer!";
	end;
	...
	if LocalPlayer():GetPos():Distance(self:GetPos()) < EML_DrawDistance then
		...
		cam.Start3D2D(pos + ang:Up()*5, ang, 0.045)		
			...
			draw.SimpleTextOutlined("Yodo líquido ("..self:GetNWInt("iodine")..")", "methFont", -121, 74, iodineColor, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, 1, Color(25, 25, 25, 100));	
