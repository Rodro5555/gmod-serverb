...
function ENT:Draw()
	...
	if self:GetPos():Distance(LocalPlayer():GetPos()) < 500 then -- How far people can see printer UI
		...
		cam.Start3D2D(Pos + Ang:Up() * 10.8 + Ang:Forward() * -15 + Ang:Right() * -16.1, Ang, 0.11)
			...
			draw.RoundedBox( 0, 0, 225, 275, 50, Color(255, 255, 0, 255) )
			draw.TexturedQuad
			{
				texture = surface.GetTextureID "gui/gradient_up",
				color = Color(252, 194, 0, 255),
			...
			draw.RoundedBox( 0, 139, 167, 126, 50, Color(252, 194, 0, 255) )
		...
		-- 	surface.SetDrawColor(Color(252, 194, 0, 255))
