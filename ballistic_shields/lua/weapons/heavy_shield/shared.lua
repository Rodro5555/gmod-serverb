...
SWEP.PrintName = "Escudo pesado"
SWEP.Author	= "D3G"
SWEP.Instructions = "IZQ - Romper prop | DER - Alternar visibilidad"
SWEP.Spawnable = true
SWEP.AdminOnly = true
...
for k, v in pairs(RPExtraTeams) do
	if v.customCheck then
		v.customCheck = nil
		v.CustomCheckFailMsg = nil
	end
end
-- if RPExtraTeams and TEAM_SWAT and RPExtraTeams[tonumber(TEAM_SWAT)] then
-- 	print("team_swat max: ", RPExtraTeams[tonumber(TEAM_SWAT)].max)
-- 	RPExtraTeams[tonumber(TEAM_SWAT)].max = 5
-- end
...
function SWEP:DrawHUD()
	...
	if(!IsValid(tr.Entity) || !DarkRP) then // || !(tr.Entity:isDoor())) then
...
function SWEP:PrimaryAttack()
	...
	if SERVER then
		if ent.isFadingDoor && !ent.fadeActive && bshields.config.breachfdoors then
			...
			if not ent:isDoor() and not entOwner then
				-- forbidden sound
			...
			timer.Simple(3.4, function()
				...
				if ent:isDoor() and ent:GetClass() != "prop_door_rotating" then
					ent:keysUnLock()
					ent:Fire("open", "", 0)
					...
					prop:CPPISetOwner(entOwner)
					prop:SetCollisionGroup(COLLISION_GROUP_NONE)
					prop:SetMoveType(MOVETYPE_VPHYSICS)
					prop:SetSolid(SOLID_NONE)//SOLID_BBOX) 
					prop:SetPos(ent:GetPos() + Vector(0,0,1))
					...
					prop:Spawn()
					prop:GetPhysicsObject():AddVelocity(direction*700)
					if ent:isDoor() then
						ent.phys_door = prop
					end
				end
