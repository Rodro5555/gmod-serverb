    ...
    hook.Add("HUDPaint", "gpoker_hudPaint" .. self:EntIndex(), function()
        ...
        if self:BeingLookedAtByLocalPlayer() and self:GetPos():Distance(LocalPlayer():GetPos()) < 128 and self:GetGameState() < 1 then
            if !self:getPlayerKey(LocalPlayer()) then
                ...
                local plyHeader = "Jugadores: "
                local startHeader = "Valor inicial: "
                local entryHeader = "Cuota de entrada: "
                ...
                local text = "Prensa [" .. string.upper(input.LookupBinding("+use")) .. "] para entrar."
                if !canJoin then text = "No puedo unirme, ya estoy en una partida." end
                if #self.players >= self:GetMaxPlayers() and (!self:GetBotsPlaceholder()) then text = "No puedo unirme - partido LLENO" end
        ...
        if self.players[self:getPlayerKey(LocalPlayer())] then
            ...
            if self:GetGameState() > 0 then
                ...
                surface.SetFont("gpoker_header")
                local money, bet, pot = gPoker.betType[self:GetBetType()].name .. ": ", "Apuesta: ", "Pozo: "
...
function ENT:openEntryFeeDerma()
    ...
    win:SetTitle("Pagar la cuota de entrada")
    win:SetSize(w, h)
    win:Center()
    win:SetVisible(true)
...
function ENT:openBettingDerma(check, curbet)
    ...
    if check then
        ...
        bet.DoClick = function()
            ...
            betAmount:SetPos(-20, 0)
            betAmount:CenterVertical()
            betAmount:SetSize(w-(h+40)/2, 25)
            ...
            betButton:SetSize((h+40)/2,(h-20)/2)
            betButton:SetPos(w - (h+40)/2,20)
            ...
            backButton:SetSize((h+40)/2,(h-20)/2)
            backButton:SetPos(w - (h+40)/2, 20 + (h-20)/2)
...
function ENT:openLeaveRequest()
    ...
    win:SetSize(w,h)
    win:Center()
    win:SetTitle("¿Quieres dejar el partido?")
    win:SetDraggable(false)
    win:ShowCloseButton(false)
    win.Paint = function(self, w, h)
        local outSize = 2
